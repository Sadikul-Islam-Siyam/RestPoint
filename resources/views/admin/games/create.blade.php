...
<div class="bg-slate-950 py-12 min-h-screen">
    <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-extrabold tracking-tight text-white font-display">Add Game to Library</h1>
            <p class="mt-1 text-sm text-slate-400">Fill in details for the game. Standard categories will seed automatically.</p>
        </div>
            ...
            <form action="{{ route('admin.games.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="genre" id="genre" required value="{{ old('genre') }}" placeholder="e.g. Action RPG, Rogue-like"
                               class="mt-1.5 block w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-slate-100 placeholder-slate-500 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500 sm:text-sm">
                        @error('genre')
                            <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" name="developer" id="developer" required value="{{ old('developer') }}" placeholder="e.g. FromSoftware"
                               class="mt-1.5 block w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-slate-100 placeholder-slate-500 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500 sm:text-sm">
                        @error('developer')
                            <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="release_date" id="release_date" required value="{{ old('release_date') }}"
                    ...
                    <div>
                        ...
                        <input type="url" name="trailer_url" id="trailer_url" value="{{ old('trailer_url') }}" placeholder="https://www.youtube.com/watch?v=..."
                               class="mt-1.5 block w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-3 text-slate-100 placeholder-slate-500 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500 sm:text-sm">
                        @error('trailer_url')
                            <span class="text-rose-500 text-xs mt-1 block">{{ $message }}</span>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                    <div>
                        <label for="cover_image" class="block text-sm font-medium text-slate-300">Cover Art (3:4 ratio recommended)</label>
                    ...
                    <div>
                        <label for="banner_image" class="block text-sm font-medium text-slate-300">Banner Background (Landscape)</label>
                        ...
                        @foreach(['Steam', 'PlayStation', 'Xbox', 'Epic Games', 'Nintendo Switch'] as $index => $store)
                            <div class="flex flex-col sm:flex-row gap-4 items-center bg-slate-950/20 p-3 rounded-xl border border-slate-850">
                                ...
                                <input type="url" name="stores[{{ $index }}][url]" placeholder="https://store.example.com/app/{{ strtolower($store) }}..."
                                       value="{{ old("stores.$index.url") }}"
                                       class="block w-full rounded-xl border border-slate-800 bg-slate-950 px-4 py-2.5 text-slate-100 placeholder-slate-600 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500 sm:text-sm">
                            </div>
                        @endforeach
                    </div>
                </div>
